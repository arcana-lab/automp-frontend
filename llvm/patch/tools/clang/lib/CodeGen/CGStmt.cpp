...
// PRAGMA-NOTE: include note codegen extensions
#include "../../tools/hobbit-tricks/pragma-note/codegen.h"
...
CodeGenFunction::EmitCompoundStmtWithoutScope(const CompoundStmt &S,
  ...
  // PRAGMA-NOTE(Simone): turn on metadata emmission for each basic block
  bool has_note = note::codegen::EnableInsertHelper(
    (void *)(&S),
    getContext(),
    getLLVMContext()
  );
  ...
  // PRAGMA-NOTE(Simone): turn off metadata emission for each basic block
  if (has_note) note::codegen::DisableInsertHelper(getContext());
...
void CodeGenFunction::EmitWhileStmt(const WhileStmt &S,
  ...
  // PRAGMA-NOTE: enable insert helper for loop, starting at condition
  bool has_note = note::codegen::EnableInsertHelper(
    (void *)(&S),
    getContext(),
    getLLVMContext()
  ...
  // PRAGMA-NOTE(Simone): disable insert helper after loop is emitted
  if (has_note) note::codegen::DisableInsertHelper(getContext());
}
...
void CodeGenFunction::EmitDoStmt(const DoStmt &S,
  ...
  // PRAGMA-NOTE: enable insert helper for loop, starting some setup
  bool has_note = note::codegen::EnableInsertHelper(
    (void *)(&S),
    getContext(),
    getLLVMContext()
...
void CodeGenFunction::EmitForStmt(const ForStmt &S,
  ...
  // PRAGMA-NOTE: enable insert helper for loop, starting with for.cond
  /* PRAGMA-NOTE: if this is enabled earlier, the br into the loop is
   * annotated! That's not correct behavior. It must be tied to the call
   * to EmitBlock(CondBlock) above, which presumably means that EmitBlock
   * not only emits the next block, but also emits the TerminatorInst of
   * the current block. (Which mroe-or-less makes sense, but still
   * annoying and non-obvious to reverse engineer at the use-site.)
   */
  bool has_note = note::codegen::EnableInsertHelper(
    (void *)(&S),
    getContext(),
