...
// PRAGMA-NOTE(Simone): insert helper
...
void CodeGenFunction::EmitStaticVarDecl(const VarDecl &D,
                                      llvm::GlobalValue::LinkageTypes Linkage) {
  /* PRAGMA-NOTE(Simone): IF we were going to annotate variable allocation
   * instructions, this would be the place to do it for all global
   * variables. See discussion in WORKLOG entry for 2019-05-24.
   */
...
void CodeGenFunction::EmitAutoVarDecl(const VarDecl &D) {
  ...
  /* PRAGMA-NOTE(Simone): CodeGenFunction::InsertHelper is not called for
   * these AllocaInst emissions for some reason. Invoke our helper
   * manually for these, when the Helper is enabled.
   */
  if (note::codegen::isInsertHelperEnabled(
    getContext(),
    note::Annotation::Scope::Function
  )) {
    llvm::errs()
      << "note:EmitAutoVarDecl: manually handling variable"
      << " function @ " << CurFuncDecl->getCanonicalDecl()
      << " variable @ " << D.getCanonicalDecl()
      << "\n";
    note::codegen::InsertHelper(
      llvm::dyn_cast<llvm::AllocaInst>(emission.Addr.getPointer()),
      getContext(),
      getLLVMContext(),
      note::Annotation::Scope::Function
    );
