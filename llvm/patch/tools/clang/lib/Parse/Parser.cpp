...
// PRAGMA-NOTE: include acceptors (note::accepts_node)
#include "../../tools/hobbit-tricks/pragma-note/acceptors.h"
// PRAGMA-NOTE: include annotation type &c.
#include "../../tools/hobbit-tricks/pragma-note/annotation.h"
...
bool Parser::ParseTopLevelDecl(DeclGroupPtrTy &Result, bool IsFirstDecl) {
  ...
  // PRAGMA-NOTE(Simone): possibly-used note::annotation
  note::Annotation * pragma_note_annotation = nullptr;

  Result = nullptr;
  switch (Tok.getKind()) {
  // PRAGMA-NOTE(Simone): handle annot_ext_note
  case tok::annot_ext_note: {
    // Get our Note Annotation
    if (Tok.getAnnotationValue() == NULL) {
      llvm::errs()
       << "note:ParseTopLevelDecl:"
       << " ERROR: annotation value is NULL";
      return false;
    }
    pragma_note_annotation = static_cast<note::Annotation *>(
      Tok.getAnnotationValue()
    );

    llvm::outs()
      << "note:ParseTopLevelDecl:"
      << " found annot_ext_note annotation token!"
      << "\n\tannotation @ " << pragma_note_annotation
      << "\n";

    // Get rid of the annot_ext_note
    ConsumeAnnotationToken();
    // FIXME?(Simone): should be `goto default`?
    break;
  }
  ...
  // PRAGMA-NOTE(Simone): if a note annotation was parsed, save it
  if (pragma_note_annotation != nullptr) {
    DeclGroupRef const DG = Result.get();
    if (DG.isSingleDecl()) {
      Decl const * D = DG.getSingleDecl();
      Decl const * CanonD = D->getCanonicalDecl();
      if (note::accepts_node(CanonD)) {
        /* NOTE(Simone): Mark the annotation as function-scope if it's
         * applied to a FunctionDecl! This way it is possible to
         * selectively apply function-scoped metadata.
         */
        if (isa<FunctionDecl>(CanonD)) {
          pragma_note_annotation->scope = note::Annotation::Scope::Function;
        }
        void * node = (void *) CanonD;
        llvm::outs()
          << "note: accepted D @ " << D << ";"
          << " annotating canonical decl CD @ " << CanonD << "\n";
        Actions.Context.NoteDSLAnnotations[node] = pragma_note_annotation;
      }
    } else {
      llvm::errs()
        << "note: cannot apply annotation to declgroup;"
        << " it is not a single decl!";
    }
  }
