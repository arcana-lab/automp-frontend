...
Retry:
  ...
  switch (Kind) {
  // PRAGMA-NOTE(Simone): handle annotation token
  case tok::annot_ext_note:
    {
      // Get our Note Annotation
      if (Tok.getAnnotationValue() == NULL) {
        llvm::errs()
         << "note:ParseStatementOrDeclarationAfterAttributes:"
         << " ERROR: annotation value is NULL";
        return StmtError();
      }
      note::Annotation * annotation
        = static_cast<note::Annotation *>(Tok.getAnnotationValue());
      ...
      // Parse subsequent statement
      MaybeParseCXX11Attributes(Attrs);
      StmtResult SR = ParseStatementOrDeclarationAfterAttributes(
        Stmts, StmtCtx, TrailingElseLoc, Attrs
      );

      // Get a Stmt from the result
      Stmt * S = SR.get();

      // If it's accepted, then put the node in the note map.
      if (note::accepts_node(S)) {
        void * node = S;
        llvm::outs() << "note: accepted S @ " << S << "\n";
        Actions.Context.NoteDSLAnnotations[node] = annotation;
      }

      // Return the StmtResult of the subsequent statement.
      return SR;
    }
